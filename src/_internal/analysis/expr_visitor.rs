...
impl ExprVisitor {
    pub(crate) fn rename_column_source(
        ...
        use squawk_syntax::ast::{CallExpr, FieldExpr, NameRef, SourceFile, Target};
        let prefix = "SELECT ";
        ...
        for name in expr.syntax().descendants().filter_map(NameRef::cast) {
            if name.text() != from {
                continue;
            }
        ...
        let replacement = format!("\"{}\"", to.replace('"', "\"\""));
