...
mod tests {
    ...
    fn role_membership_set_option_is_extracted() {
        ...
        assert_eq!(
            grant.target,
            crate::_internal::analysis::facts::GrantTarget::Roles(vec!["parent_role".into()])
        );
        assert_eq!(
            grant.role_options,
            vec![crate::_internal::analysis::facts::RoleMembershipOptionFact::Set(true)]
        );
        ...
        for (sql, expected) in [
            (
                ...
                crate::_internal::analysis::facts::RoleMembershipOptionFact::Admin(true),
            ),
            (
                "GRANT parent_role TO member_role WITH INHERIT FALSE;",
                crate::_internal::analysis::facts::RoleMembershipOptionFact::Inherit(false),
            ),
        ] {
    ...
    fn test_set_search_path_default() {
        ...
        match facts.unwrap() {
            StatementFact::SetSearchPath {
                target: crate::_internal::analysis::facts::SearchPathTarget::Default,
                local: false,
            } => {}
    ...
    fn test_set_search_path_preserves_user_placeholder() {
        ...
        assert_eq!(
            facts,
            StatementFact::SetSearchPath {
                target: crate::_internal::analysis::facts::SearchPathTarget::Schemas(vec![
                    "$user".into(),
                    "public".into(),
                ]),
                local: false,
    ...
    fn global_object_identifiers_follow_postgresql_case_rules() {
        ...
        assert!(matches!(
            database.action,
            AlterDatabaseAction::OwnerChange(crate::_internal::analysis::facts::RoleFact::Named { name, .. })
                if name == "appowner"
        ));
    }
            ...
            StatementFact::AlterPublication(fact)
                if matches!(
                    ...
                    AlterPublicationActionFact::SetOptions(options)
                        if options == &[crate::_internal::analysis::facts::AttributeFact {
                            name: "publish".into(),
    ...
    fn aggregate_commands_extract_shared_routine_identities() {
        ...
        assert!(matches!(
            alter.action,
            crate::_internal::analysis::facts::AlterFunctionAction::Rename { ref to, .. }
                if to == "Combined"
        ));
    ...
    fn test_alter_schema() {
        ...
        match facts.unwrap() {
            StatementFact::AlterSchema { name, action } => {
                assert_eq!(name.name.resolve(), "my_schema");
                assert!(
                    matches!(action, crate::_internal::analysis::facts::AlterSchemaActionFact::RenameTo { new_name } if new_name.resolve() == "new_schema")
                );
            }
    ...
    fn set_role_named_produces_set_role_fact() {
        ...
        assert_eq!(
            role,
            Some(crate::_internal::analysis::facts::RoleFact::Named {
    ...
    fn squawk_264_typed_view_sequence_policy_and_function_children_preserve_facts() {
        ...
        assert!(matches!(
            ...
            StatementFact::AlterSequence {
                action: crate::_internal::analysis::facts::AlterSequenceActionFact::OwnedBy(None),
                ..
            }
        ));
        assert!(matches!(
            ...
            StatementFact::CreatePolicy {
                command: crate::_internal::analysis::facts::PolicyCommand::Select,
                ..
            }
        ));
        ...
        assert!(function.options.iter().any(|option| matches!(
            option,
            crate::_internal::analysis::facts::FuncOptionFact::Volatility(
    ...
    fn function_options_use_typed_targets_and_decode_literals() {
        ...
        assert!(function.options.iter().any(|option| matches!(
            option,
            crate::_internal::analysis::facts::FuncOptionFact::Parallel(value) if value == "safe"
        )));
        assert!(function.options.iter().any(|option| matches!(
            option,
    ...
    fn squawk_264_typed_replication_and_privilege_children_preserve_facts() {
        ...
        assert!(matches!(
            ...
            StatementFact::Grant(grant)
                if grant.privileges == crate::_internal::analysis::facts::PrivilegeSpec::All
                    && grant.granted_by
                        == Some(crate::_internal::analysis::facts::RoleFact::Named {
                            name: "admin".into(),
                            via_legacy_group_syntax: false,
