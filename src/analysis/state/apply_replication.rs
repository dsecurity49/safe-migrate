...
use crate::model::replication::{PublicationOverlay, SubscriptionOverlay};
...
impl AnalysisState {
    ...
    pub(super) fn apply_alter_publication(
        ...
        match &p.action {
            AlterPublicationActionFact::AddObjects(additions) => {
                ...
                let crate::analysis::facts::PublicationScope::Explicit(mut objects) = current_scope
                else {
                    return MutationResult::Conflict {
        ...
        if let Some(to) = rename_to {
            ...
            let Some(crate::model::replication::PublicationOverlay::Present(mut publication)) =
    ...
    pub(super) fn apply_alter_subscription(
        ...
        self.local.generation_counter += 1;
        let new_gen = self.local.generation_counter;
        use crate::analysis::facts::{AlterSubscriptionActionFact, SubscriptionPublicationMode};
        let mut rename_to = None;
        match &s.action {
        ...
        if let Some(to) = rename_to {
            ...
            let Some(crate::model::replication::SubscriptionOverlay::Present(mut subscription)) =
