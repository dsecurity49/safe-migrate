...
closure is fully modeled. It also carries PostgreSQL-selected PK/FK, PK/PK, and
FK/FK equality-operator identities for each synchronized foreign-key column,
and rejects incomplete operator evidence rather than treating it as exact.
